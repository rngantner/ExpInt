...
def processOdeArgs(**kwargs):
    defaults = {'normcontrol':'on',    # ('on','off')
                'nonnegative':None,        # index array describing components that should be nonnegative
                'reltol':1e-5,            # relative tolerance (can be array of same dimension as y0)
                'abstol':1e-5,            # absolute tolerance
                'outputsel':None,        # which components to save
                'outputsave':None,        # save output every outputsave steps
                'initialstep':None,        
                'maxstep':None,            
                'mass':None,            
                'stats':'off',            # statistics
                'verbose':'off'            # print statistics or not
                }
    
    if len(kwargs) > 0:
        for i in kwargs: defaults[i.lower()] = kwargs[i]
    
    # accept True instead of 'on' for certain options
    opts = ['stats', 'normcontrol', 'verbose']
    for o in opts:
        if defaults[o] == False or defaults[o] == 'off':
            defaults[o] = 'off'
        elif defaults[o] == True or defaults[o] == 'on':
            defaults[o] = 'on'
    return defaults

def ode45(vfun, vslot, vinit, **kwargs):
    """ode45 integrates vfun over vslot with initial value vinit.
    vfun:   Function accepting arguments t,y in that order.
    vslot:  Tuple containing start and end time. ex: (t0, t1)
    vinit:  Initial function value (y_0)
    kwargs: keyword arguments specifying additional parameters. See output of processOdeArgs.
    returns: tuple (t,Y), where Y is a matrix containing y(t_i) in its columns
    """
...
#        warning ('OdePkg:InvalidArgument', 'Option "reltol" not set, new value %f is used'%vodeoptions['reltol'])
        ...
        warning ('OdePkg:InvalidArgument', 'Option "reltol" will be ignored if fixed time stamps are given')
...
#        warning('OdePkg:InvalidArgument', 'Option "abstol" not set, new value %f is used'%vodeoptions['abstol'])
        ...
        warning('OdePkg:InvalidArgument', 'Option "abstol" will be ignored if fixed time stamps are given')
    ...
    if vodeoptions['nonnegative'] != None:
        if vodeoptions['mass'] == None:
            ...
            warning('OdePkg:InvalidArgument', 'Option "nonnegative" will be ignored if mass matrix is set')
    else:
        vhavenonnegative = False
...
#        warning ('OdePkg:InvalidArgument', 'Option "maxstep" not set, new value %f is used'%vodeoptions['maxstep'])
    ...
    vb5 = array([16./135, 0, 6656./12825, 28561./56430, -9./50, 2./55])
    vc = sum(va, 1)
    
    # The solver main loop - stop if the endpoint has been reached
    vcntloop = 2; vcntcycles = 1; vu = vinit.copy()
    d = len(vinit); vk = zeros([6,d]); vcntiter = 0
...
#                vdelta = vtau * (0.4 ** (1. / vpow))
            ...
            if size(vdelta) > 1: vdelta[vdelta == 0] = max(vtau) * (0.4 ** (1. / vpow))
...
#            print 'vtau',vtau,'vdelta',vdelta
            ...
            if vdirection == 1:
                ...
                vstepsize = max([vodeoptions['maxstep'], max(0.8 * vstepsize * (vtau / vdelta) ** vpow)])
    ...
    if vodeoptions['stats'] == 'on':
        ...
        if vodeoptions['verbose'] == True or vodeoptions['verbose'] == 'on':
    ...
    if vhaveoutputselection:
        vretvalresult = vretvalresult.transpose()[vodeoptions['outputsel']].transpose()
    if vhavestats:
        return (vretvaltime, vretvalresult, {"accept":vnsteps,"reject":vnfailed,"RHSevals":vnfevals})
